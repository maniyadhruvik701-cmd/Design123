...
export default function Completed() {
  ...
  const completedDesigns = designs.filter(d => 
    d.status === 'completed' && !d.stockStatus &&
    ...
    <div>
      ...
      <div>
        ...
        {completedDesigns.length === 0 ? (
          <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)', marginBottom: '2rem' }}>
            No completed designs found.
          </div>
        ) : (
          <div className="design-grid" style={{ marginBottom: '3rem' }}>
            {completedDesigns.map(design => (
              <div key={design.id} className="design-card" style={{ opacity: 0.8 }}>
                <div 
      ...
      <div style={{ marginTop: '3rem', borderTop: '1px solid var(--border-color)', paddingTop: '2rem' }}>
        ...
        {stockInDesigns.length === 0 ? (
          <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)', marginBottom: '2rem' }}>
            No stock in designs found.
          </div>
        ) : (
          <div className="design-grid" style={{ marginBottom: '3rem' }}>
            {stockInDesigns.map(design => (
              <div key={design.id} className="design-card" style={{ opacity: 0.8, borderColor: '#86efac' }}>
                <div 
                  className="design-image-container" 
                  onClick={() => setSelectedStockDesignId(design.id)}
                  style={{ cursor: 'pointer' }}
      ...
      <div style={{ marginTop: '3rem', borderTop: '1px solid var(--border-color)', paddingTop: '2rem' }}>
        ...
        {stockOutDesigns.length === 0 ? (
          <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>
            No stock out designs found.
          </div>
        ) : (
          <div className="design-grid">
            {stockOutDesigns.map(design => (
              <div key={design.id} className="design-card" style={{ opacity: 0.8, borderColor: '#fca5a5' }}>
                <div 
                  className="design-image-container" 
                  onClick={() => setSelectedStockDesignId(design.id)}
                  style={{ cursor: 'pointer' }}
                ...
                <div className="design-info">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div className="design-sku">{design.sku}</div>
                    <span style={{ color: '#ef4444', display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.85rem', fontWeight: 600 }}>
                      Stock Out
                    </span>
                  </div>
                  <div className="design-platform">Platform: {design.platform}</div>
